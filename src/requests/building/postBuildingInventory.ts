...
export async function postBuildingInventory(body: IInventoryBody, options: any): Promise<any> {
    ...
    if (options.onlyCounts !== undefined) params.append("onlyCounts", String(options.onlyCounts));
    try {
        const response = await spinalAPI.post<any>(`${url}?${params.toString()}`, body);
        return response.data;
    } catch (error) {
        console.error("Erreur lors de la récupération de l’inventaire du Building:", error);
        throw error;
    }
}
